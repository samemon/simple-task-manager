...
- Restart app: a sidebar button (`restartApp()` → `POST /api/restart`) relaunches the Python process so it reloads `app.py` from disk (to pick up new code). It spawns a detached retry-launcher of the original command and `os._exit(0)`s to free the port (can't `os.execv` in place — werkzeug's listening socket would be inherited and the rebind would fail); the page polls `/api/status` and reloads when it's back. `KAAMKAAJ_NO_BROWSER=1` suppresses the browser pop on the relaunch.
